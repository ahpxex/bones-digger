...
/**
 * Provider-agnostic identification engine. The schema, the JSON-normalisation
 * gauntlet, and the dual-channel (fast + thinking) fusion live here so that any
 * OpenAI-compatible VLM (Qwen via DashScope, Doubao via Volcengine ARK, …) only
 * needs to supply a `ChannelCaller` that performs the actual HTTP request.
 */

export const SPECIES_ENUM = ANALYSIS_SPECIES_LIST;
...
export function knowledgeToPrompt(cards: KnowledgeCard[]): string {
  return cards
    .map((c, i) => `${i + 1}. ${c.species}·${c.position}：${c.features.join("；")}`)
    .join("\n");
}
...
export function buildSystemPrompt(
  ...
  return [
    ...
    "}",
    "",
    "- evidence 数组必须给出 3–5 条，每一项都必须包含 key / observation / referenceFeature / weight 四个字段，并且必须带上 region 对象（归一化坐标），用于在图像上高亮对应观察区域。",
    "- 即使图像不完美或残缺，也要基于可见的形态要素给出至少 3 条证据并各自标注 region，绝不能返回空数组。",
...
/**
 * Parse a raw model reply into a validated AnalysisResponse, applying the full
 * normalisation gauntlet (synonym keys, array-form regions, closed-set ranking,
 * unknown-label coercion). Throws with a rich diagnostic on unrecoverable input.
 */
export function parseAnalysisText(
  ...
  try {
    ...
    throw new Error(
      `${modelLabel} returned invalid JSON:\n---raw---\n${text.slice(0, 600)}\n---extracted---\n${jsonText.slice(0, 600)}`,
    );
  }
  ...
  const speciesFromKnownLabel = (
    value: unknown,
  ): AnalysisSpecies | undefined => {
    if (typeof value !== "string") return undefined;
    const raw = value.trim();
    const lowered = raw.toLowerCase();
    if (raw === "马" || lowered.includes("equus")) return "马";
    if (
      raw === "黄牛" ||
      raw === "牛" ||
      raw === "家牛" ||
      raw === "水牛" ||
      lowered.includes("bos") ||
      lowered.includes("bubalus") ||
      raw.includes("牛")
    ) {
      return "黄牛";
    }
  ...
  if (!result.success) {
    throw new Error(
      `${modelLabel} JSON did not match schema:\n${JSON.stringify(result.error.issues, null, 2)}\n---parsed---\n${JSON.stringify(parsed, null, 2)}\n---raw text---\n${text}`,
    );
  }
  return result.data;
}

export function verdictFromResp(resp: AnalysisResponse): Verdict {
...
/** Calls one VLM channel for the given model with thinking on/off. */
...
function assembleAnalysis(
  finalResp: AnalysisResponse,
  finalVerdict: Verdict,
  retrieved: KnowledgeCard[],
  thinkingResp: AnalysisResponse | null,
  channelVerdicts: { realtime: Verdict; thinking?: Verdict },
): PartialAnalysis {
  const cards: KnowledgeCard[] = [];
  const topCard = cardFor(finalVerdict.species, finalVerdict.position);
  ...
  return {
    ...
    outOfDistribution: false,
    channelVerdicts: {
      realtime: channelVerdicts.realtime,
      thinking: channelVerdicts.thinking,
    },
    subjectBox: finalResp.subjectBox,
    featureRegions: finalResp.evidence
      .filter((e) => e.region)
      .map((e) => ({
        ...(e.region as { x: number; y: number; w: number; h: number }),
        label: e.key,
        weight: e.weight,
      })),
...
/**
 * The dual-channel pipeline shared by every OpenAI-compatible VLM provider:
 *   1. fast (realtime) channel — thinking disabled, runs on every image
 *   2. thinking channel — runs only when realtime confidence is low
 *   3. fusion — agree → boost confidence; disagree → trust thinking
 */
export async function runDualChannel(opts: {
  realtimeModel: string;
  thinkingModel: string;
  callChannel: ChannelCaller;
  retrieved: KnowledgeCard[];
  providerTag: string;
  /** When false, the slow deep pass is skipped entirely (single fast channel). */
  thinkingEnabled?: boolean;
}): Promise<PartialAnalysis> {
  const { realtimeModel, thinkingModel, callChannel, retrieved, providerTag } =
    opts;
  const thinkingEnabled = opts.thinkingEnabled ?? true;
  // Read at call time (not module scope) so the Workers runtime has populated env.
  const threshold = Number(
    process.env.THINKING_CHANNEL_THRESHOLD ?? `${LOW_CONFIDENCE_THRESHOLD}`,
  );

  // Step 1 — realtime channel (fast, low latency)
  const realtimeResp = await callChannel(realtimeModel, false);
  const realtimeVerdict = verdictFromResp(realtimeResp);

  // Step 2 — thinking channel runs for low-confidence samples (when enabled).
  const shouldThink =
    thinkingEnabled && realtimeVerdict.confidence < threshold;

  let thinkingResp: AnalysisResponse | null = null;
  let thinkingVerdict: Verdict | undefined;
  if (shouldThink) {
    try {
      thinkingResp = await callChannel(thinkingModel, true);
      thinkingVerdict = verdictFromResp(thinkingResp);
    } catch (err) {
      console.warn(
        `[${providerTag}] thinking channel failed, keeping realtime verdict:`,
  ...
  if (thinkingResp && thinkingVerdict) {
    ...
    if (agree) {
      ...
      finalVerdict = {
        ...
        confidence: Math.min(
          0.99,
          0.5 * (realtimeVerdict.confidence + thinkingVerdict.confidence) + 0.08,
        ),
      };
    } else {
  ...
  return assembleAnalysis(finalResp, finalVerdict, retrieved, thinkingResp, {
