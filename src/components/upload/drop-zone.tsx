...
export function DropZone() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      fd.append("hints", hintsRef.current?.value ?? "");
      const { id } = await analyzeBone({ data: fd });
      await router.navigate({ to: "/analyze/$id", params: { id } });
    } catch (err) {
      setPending(false);
      setError(
        err instanceof Error ? err.message : "鉴定失败，请稍后重试。",
      );
    }
  }
  ...
  return (
    <form className="flex flex-col gap-6" onSubmit={handleSubmit}>
      ...
      <div className="flex items-center justify-between gap-6">
        ...
        >
          {isPreparing ? "处理中 …" : pending ? "鉴定中 …" : "开始鉴定"}
        </button>
      </div>
    </form>
  );
  ...
  return (
    ...
    >
      ...
      <rect
        ...
        strokeWidth="1.5"
      />
      <circle cx="32" cy="34" r="10" stroke="currentColor" strokeWidth="1.5" />
